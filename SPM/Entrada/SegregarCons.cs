...
namespace APP {
  ...
  public partial class SegregarCons : FrmBaseGrid {
    ...
    public override void InicializarFiltros(bool guardaPersistencia = true) {
      ...
      Filtro.Agregar(new FiltroDropDown(ddlPlanta, chkPlanta, TIPO_OPERADOR.Igual, "Segregacion.IdPlanta", "Planta", TIPO_DATO.Entero));

      Filtro.Agregar(new FiltroTextBox(txtNumero, chkNumero, ddlNumero, "Segregacion.Numero", "Número", TIPO_DATO.Texto));
      Filtro.Agregar(new FiltroTextBox(txtCliente, chkCliente, ddlCliente, "Entidad.Nombre", "Cliente", TIPO_DATO.Texto));


      Filtro.Agregar(new FiltroTextBox(txtCodigo, chkCodigo, ddlCodigo, "TarjaIngreso.id_insumo", "Código", TIPO_DATO.Texto, false, "Exists(Select 1 From TarjaIngreso Inner join SegregacionDet On TarjaIngreso.Id = SegregacionDet.IdTarjaIngreso Where SegregacionDet.IdSegregacion = Segregacion.Id and &&)"));
      Filtro.Agregar(new FiltroTextBox(txtNombrePTT, chkNombrePTT, ddlNombrePTT, "TarjaIngreso.nombre_insumo", "Descripción", TIPO_DATO.Texto, false, "Exists(Select 1 From TarjaIngreso Inner join SegregacionDet On TarjaIngreso.Id = SegregacionDet.IdTarjaIngreso Where SegregacionDet.IdSegregacion = Segregacion.Id and &&)"));
      Filtro.Agregar(new FiltroTextBox(txtTarja, chkTarja, ddlTarja, "TarjaIngreso.id_taring", "Tarja origen", TIPO_DATO.Entero, false, "Exists(Select 1 From TarjaIngreso Inner join SegregacionDet On TarjaIngreso.Id = SegregacionDet.IdTarjaIngreso Where SegregacionDet.IdSegregacion = Segregacion.Id and &&)"));

      Filtro.Agregar(new FiltroDatePicker(dtpFechaCreaDesde, chkFechaCreaDesde, ddlFechaCreaDesde, "Segregacion.FechaCrea", "Fecha creación desde", TIPO_DATO.Fecha, TIPO_OPERADOR.MayorIgual));
      Filtro.Agregar(new FiltroDatePicker(dtpFechaCreaHasta, chkFechaCreaHasta, ddlFechaCreaHasta, "Segregacion.FechaCrea", "Fecha creación hasta", TIPO_DATO.Fecha, TIPO_OPERADOR.MenorIgual));
      Filtro.Agregar(new FiltroListBox(lstEstado, chkEstado, ddlEstado, "Segregacion.IdEstado", "Estado", TIPO_DATO.Entero));
    ...
    private bool Eliminar() {
      return null != this.XEjecutarProcedimiento(null, "SegregacionDel", TomarId(dtgPrincipal));
    }
    ...
    public override void CargarPagina(int pagina, int pagTamano, int cantidad) {
      ...
      var dtTabla = Global.DatosApp.TraerTabla("SegregacionSel_Grids", objParam);
    ...
    private void tsbModificar_Click(object sender, EventArgs e) {
      Global.LlamarFormulario<Segregar>(false, TomarId(dtgPrincipal));
    }
    ...
    private void tsbNuevo_Click(object sender, EventArgs e) {
      Global.LlamarFormulario<Segregar>(false, -1);
    }
    ...
    private void dtgPrincipal_DespuesCambioFila(object sender, FilaGrillaEventArg e) {
      tsbEliminar.Enabled = e.FilaDatos != null && (ESTADO_SEGREGACION)e.FilaDatos.XEntero("IdEstado") == ESTADO_SEGREGACION.Creado;
    }
