...
namespace APP {
  ...
  public partial class Bodega : FrmBaseGrid {
    ...
    public override GUARDAR_RESULT GuardarTodo() {
      ...
      List<Registro> listaResultados = Global.DatosApp.ActualizarMasivo("Bodega", listaCambios, false);
    ...
    public override void RefrescarTodo() {
      ...
      Cursor = Cursors.WaitCursor;
      DataTable dtTabla = Global.DatosApp.TraerTabla("BodegaSel_Grids");
      Cursor = Cursors.Default;
      if (dtTabla != null) {
    ...
    private void ValidarFila(DataRow dtrFila) {
      ...
      if (string.IsNullOrWhiteSpace(dtrFila["id_planta"].ToString()))
        stbError.AppendLine("Planta es requerido.");
      
      if (string.IsNullOrWhiteSpace(dtrFila["nombre_bodega"].ToString()))
