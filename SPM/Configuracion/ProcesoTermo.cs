...
namespace APP {
  ...
  public partial class ProcesoTermo : FrmBaseGrid {
    ...
    public override GUARDAR_RESULT GuardarTodo() {
      ...
      List<Registro> listaResultados = Global.DatosApp.ActualizarMasivo("ProcesosTermo", listaCambios, false);
    ...
    public override void RefrescarTodo() {
      ...
      Cursor = Cursors.WaitCursor;
      DataTable dtTabla = Global.DatosApp.TraerTabla("ProcesosTermoSel_Grids");
      Cursor = Cursors.Default;
      if (dtTabla != null) {
    ...
    private void ValidarFila(DataRow dtrFila) {
      ...
      if (string.IsNullOrWhiteSpace(dtrFila["nombre_procesoTermo"].ToString()))
    ...
    private void dtgPrincipal_AntesInsertar(object sender, FilaEventArg e) {
      e.Fila["activo_procesoTermo"] = true;
      ValidarFila(e.Fila);
    }
