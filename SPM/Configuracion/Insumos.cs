...
namespace APP {
  ...
  public partial class Insumos : FrmBaseGrid {
    ...
    public override GUARDAR_RESULT GuardarTodo() {
      ...
      List<Registro> listaResultados = Global.DatosApp.ActualizarMasivo("Insumos", listaCambios, false);
    ...
    public override void RefrescarTodo() {
      ...
      Cursor = Cursors.WaitCursor;
      DataTable dtTabla = Global.DatosApp.TraerTabla("InsumosSel_Grids");
      Cursor = Cursors.Default;
      if (dtTabla != null) {
    ...
    private void ValidarFila(DataRow dtrFila) {
      ...
      if (string.IsNullOrWhiteSpace(dtrFila["id_insumo"].ToString()))
        stbError.AppendLine("Código es requerido");


      if (string.IsNullOrWhiteSpace(dtrFila["id_tipoInsu"].ToString()))
        stbError.AppendLine("Tipo es requerido");


      if (string.IsNullOrWhiteSpace(dtrFila["nombre_insumo"].ToString()))
    ...
    private void LlenarDdls() {
      colid_tipoInsu.DisplayMember = "Tipo";
      colid_tipoInsu.ValueMember = "Id";
      colid_tipoInsu.DataSource = Global.Cache.TomarVista(ClaseCache.TABLA.TipoInsumo, "Id > 0", colid_tipoInsu.DisplayMember);
    }
