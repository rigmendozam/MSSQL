...
namespace APP {
  ...
  public partial class TipoAgenda : FrmBaseGrid {
    ...
    public override GUARDAR_RESULT GuardarTodo() {
      ...
      List<Registro> listaResultados = Global.DatosApp.ActualizarMasivo("TipoAgenda", listaCambios, false);
    ...
    public override void RefrescarTodo() {
      ...
      Cursor = Cursors.WaitCursor;
      DataTable dtTabla = Global.DatosApp.TraerTabla("TipoAgendaSel_Grids");
      Cursor = Cursors.Default;
      if (dtTabla != null) {
    ...
    private void ValidarFila(DataRow dtrFila) {
      ...
      if (string.IsNullOrWhiteSpace(dtrFila["id_tipoAgenda"].ToString()))
        stbError.AppendLine("Código es requerido");


      if (string.IsNullOrWhiteSpace(dtrFila["desc_tipoAgenda"].ToString()))
