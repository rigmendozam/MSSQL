...
namespace APP {
  ...
  public partial class Planta : FrmBaseGrid {
    ...
    public override GUARDAR_RESULT GuardarTodo() {
      ...
      List<Registro> listaResultados = Global.DatosApp.ActualizarMasivo("Planta", listaCambios, false);
    ...
    private void LlenarDdls() {
      ...
      colid_comu.DisplayMember = "Comuna";
      colid_comu.ValueMember = "Id";
      colid_comu.DataSource = Global.Cache.TomarVista(ClaseCache.TABLA.Comuna, "Id > 0", colid_comu.DisplayMember);
      
      colid_ciud.DisplayMember = "Ciudad";
      colid_ciud.ValueMember = "Id";
      colid_ciud.DataSource = Global.Cache.TomarVista(ClaseCache.TABLA.Ciudad, "Id > 0", colid_ciud.DisplayMember);
    }

    public override void RefrescarTodo() {
      tsbGuardar.Enabled = false;
      Cursor = Cursors.WaitCursor;
      DataTable dtTabla = Global.DatosApp.TraerTabla("PlantaSel_Grids");
      Cursor = Cursors.Default;
      if (dtTabla != null) {
    ...
    private void ValidarFila(DataRow dtrFila) {
      ...
      if (string.IsNullOrWhiteSpace(dtrFila["nombre_planta"].ToString()))
        stbError.AppendLine("Planta es requerido.");

      if (string.IsNullOrWhiteSpace(dtrFila["dire_planta"].ToString()))
        stbError.AppendLine("Dirección es requerido.");

      if (string.IsNullOrWhiteSpace(dtrFila["id_comu"].ToString()))
        stbError.AppendLine("Comuna es requerido.");

      if (string.IsNullOrWhiteSpace(dtrFila["id_ciud"].ToString()))
        stbError.AppendLine("Ciudad es requerido.");
